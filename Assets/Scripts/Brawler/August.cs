...
public class August : PlayerSkill
{
    ...
    public override void Update()
    {
        ...
        if (Input.GetMouseButton(1) && canSuper)
        {
            ...
            else if (!isSuperFinish && addForce <= 4f)
            {
    ...
    public override void Super()
    {
        ...
        GrenadeServerRpc(addForce);
        addForce = 0;
    }

    [ServerRpc(RequireOwnership = false)]
    private void GrenadeServerRpc(float addForce, ServerRpcParams serverParam = default)
    {
        ...
        foreach (GameObject player in players)
        {
            if (player.GetComponent<PlayerSetUp>().id == (int)serverParam.Receive.SenderClientId)
            {
                ...
                grenade.GetComponent<Grenade>().InitClientRpc((int)serverParam.Receive.SenderClientId, superDamage, explodeSuperRange, explodeTime, chargeSuperPerHit, 2);
